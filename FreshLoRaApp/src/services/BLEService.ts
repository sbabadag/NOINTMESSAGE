...
export class BLEService {
        ...
        this.manager.startDeviceScan(
          ...
          (error, device) => {
            ...
            if (device) {
              ...
              // Check for ESP32 LoRa devices with enhanced pattern matching
              if (device.name && !deviceIds.has(device.id)) {
                ...
                if (isLoRaDevice) {
                  ...
                  // Enhanced station type detection
                  let stationType: 'M1' | 'M2' = 'M1';
                  if (originalName.includes('M2') || 
                      originalName === 'M2' || 
                      deviceName.includes('m2') ||
                      originalName.startsWith('M2_')) {
                    stationType = 'M2';
  ...
  private handleIncomingMessage(rawMessage: string): void {
    try {
      ...
      // Try to parse JSON first (if ESP32 sends structured data)
      let parsedMessage;
      try {
        parsedMessage = JSON.parse(rawMessage);
      } catch {
        // If not JSON, treat as plain text message
        // Parse formatted message from ESP32: "[M1M2 12:34:56] Hello World"
        const messageMatch = rawMessage.match(/\[(.*?)\s+(\d{2}:\d{2}:\d{2})\]\s*(.+)/);
        
        if (messageMatch) {
          const [, routeInfo, timeStr, messageText] = messageMatch;
          const fromStation = routeInfo.split('')[0].trim();
  ...
  async sendMessage(text: string, messageId?: string): Promise<boolean> {
    ...
    try {
      ...
      // Create message packet - format expected by ESP32
      const messagePacket = {
  ...
  getConnectedStationType(): 'M1' | 'M2' | null {
    ...
    if (name?.includes('M2') || name === 'M2') return 'M2';
    return 'M1';
  }
}
