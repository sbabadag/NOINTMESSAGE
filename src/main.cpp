// BIDIRECTIONAL MESSAGE TUNNEL - M2 STATION
// Receives LoRa messages and forwards to phone via BLE
...
// BLE Configuration for M2
const char* BLE_DEVICE_NAME = "M2";
...
// Message packet structure (must match sender)
...
class ServerCallbacks : public NimBLEServerCallbacks {
  void onConnect(NimBLEServer* pServer) {
    ...
    if (pCharStatus) {
      pCharStatus->setValue("M2_CONNECTED");
      pCharStatus->notify();
    }
  }
...
class MessageCallbacks : public NimBLECharacteristicCallbacks {
  void onWrite(NimBLECharacteristic* pCharacteristic) {
    ...
    if (value.length() > 0) {
      Serial.println("\n╔═══════════════════════════════════╗");
      Serial.println("║  📱 MESSAGE FROM PHONE (M2)       ║");
      Serial.println("╚═══════════════════════════════════╝");
      Serial.printf("Message: \"%s\"\n", value.c_str());
      Serial.println("═══════════════════════════════════\n");
      
      // Forward to LoRa (to M1)
...
void sendMessageToPhone(const char* message, uint32_t timestamp) {
  ...
  if (pCharRxMessage) {
    ...
    snprintf(formatted, sizeof(formatted), "[M1→M2 %02lu:%02lu:%02lu] %s", 
             hours % 24, minutes % 60, seconds % 60, message);
    
    pCharRxMessage->setValue(formatted);
    pCharRxMessage->notify();
    
    Serial.println("📱 Message forwarded to phone (M2)");
  }
  
  // Update status
  if (pCharStatus) {
    char status[64];
    snprintf(status, sizeof(status), "M2_MESSAGES:%u", messageCount);
    pCharStatus->setValue(status);
    pCharStatus->notify();
  }
}

// Send message FROM phone via LoRa (M2 → M1)
void sendMessageViaLoRa(const char* text) {
  if (!loraReady) {
  ...
  if (state == RADIOLIB_ERR_NONE) {
    Serial.println("✓ Message sent successfully to M1!");
      ...
      if (len > 0 && len <= sizeof(buffer)) {
        ...
        if (state == RADIOLIB_ERR_NONE && len >= sizeof(uint32_t) + sizeof(uint16_t)) {
          ...
          if (packet->messageLen > 0 && packet->messageLen <= MAX_MESSAGE_LEN) {
            ...
            // Display on serial
            Serial.println("\n╔═══════════════════════════════════╗");
            Serial.println("║  📨 MESSAGE FROM M1               ║");
            Serial.println("╚═══════════════════════════════════╝");
            Serial.printf("Message #%u\n", messageCount);
            Serial.printf("From: M1 Station\n");
