...
class MessageCallbacks : public NimBLECharacteristicCallbacks {
  void onWrite(NimBLECharacteristic* pCharacteristic) {
    ...
    if (value.length() > 0) {
      ...
      // Forward to LoRa sender (if needed for 2-way messaging)
...
void initBLE() {
  ...
  pCharTxMessage = pService->createCharacteristic(
    ...
    NIMBLE_PROPERTY::WRITE | NIMBLE_PROPERTY::WRITE_NO_RSP
  );
...
void sendMessageToPhone(const char* message, uint32_t timestamp) {
  ...
  if (pCharRxMessage) {
    ...
    snprintf(formatted, sizeof(formatted), "[%02lu:%02lu:%02lu] %s", 
             hours % 24, minutes % 60, seconds % 60, message);
    
    pCharRxMessage->setValue(formatted);
    pCharRxMessage->notify();
    
    Serial.println("📱 Message forwarded to phone");
  }
  
  // Update status
  if (pCharStatus) {
    char status[64];
    snprintf(status, sizeof(status), "MESSAGES:%u", messageCount);
...
void setup() {
  ...
  Serial.println("\n╔═══════════════════════════════════╗");
  Serial.println("║  MESSAGE TUNNEL - RECEIVER        ║");
  Serial.println("╚═══════════════════════════════════╝\n");
      ...
      if (len > 0 && len <= sizeof(buffer)) {
        ...
        if (state == RADIOLIB_ERR_NONE && len >= sizeof(uint32_t) + sizeof(uint16_t)) {
          ...
          if (packet->messageLen > 0 && packet->messageLen <= MAX_MESSAGE_LEN) {
            ...
            Serial.printf("From: Sender Station\n");
